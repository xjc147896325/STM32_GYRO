...
{
	Gyro_Info.Angular   		 = 0;
	Gyro_Info.Angular_velocity_z = 0;
	Gyro_Info.Gyro_Message		 = 0;
	Gyro_Info.Gyro_Temperature	 = 0;
	
}

void Integral_Ang(void)
{
	Gyro_Info.Angular += Gyro_Info.Angular_velocity_z * INFINITESIMAL;
}
			...
			if((Rxbuf & 0x80) == 0x00)
			{
				...
				else
				{
					if(XOR_Check(Temp, Temprature) == Rxbuf)
					{
						...
						Serial_Send(Serial1,"Gyro_Info.Angular = %f \r\n", Gyro_Info.Angular);   //OK
					}
					else
					{
						flag    = 0;
